...
Original file is located at
    https://colab.research.google.com/drive/16EGYe5JU57d6XbT1YJumt-IwrCxjMd4w
"""
...
def regression_func():
   #summer_df = pd.read_csv('/content/drive/MyDrive/CS519-Group Project/summer_products/summer-products-with-rating-and-performance_2020-08.csv')
  summer_df = pd.read_csv("summer-products-with-rating-and-performance_2020-08.csv")
  for col in summer_df.columns:
    print(col)
  #format data for regression analysis

  #drop merchant id, unnessary for regression
  summer_df = summer_df.drop(summer_df.columns[[0, 1, 4, 18, 19, 20, 22, 28, 29, 30, 31, 32, 35, 37, 38, 39, 40, 41, 42]], axis=1)
  #success_df = success_df.drop(success_df.columns[[0]], axis=1)
  #move 'totalunitssold' to the last column position position to act as the target value
  cols = list(summer_df.columns.values)
  cols.pop(cols.index('units_sold'))
  summer_df = summer_df[cols+['units_sold']]

  #fill in any NA values with zeros
  summer_df = summer_df.fillna(0)

  #Linear Regression
  X = summer_df.iloc[:,:-1].values
  y = summer_df['units_sold'].values
  ...
  #print coefficients
  print('Coefficients: ')
  cdf = pd.DataFrame(zip(summer_df.columns, lr.coef_), columns=['Attribute', 'Coefficient'])
  print(cdf)
  ...
  ax.set_xlabel('Attribute')
  ax.set_ylabel('Coefficient Value')
  ax.set_title('Linear Regression Coefficient Values to Units Sold')
  plt.xticks( rotation=90)
  plt.axhline(y=0, linewidth=1, color='k', linestyle='dashed')
  ...
  plt.show()
    ...
    #Nonliner - Random Forest Regressor - max_depth = 10
  X = summer_df.iloc[:,:-1].values
  y = summer_df['units_sold'].values
  ...
  #print coefficients
  print('Feature Importance: ')
  cdf = pd.DataFrame(zip(summer_df.columns, forest.feature_importances_), columns=['Attribute', 'Feature Importance'])
  print(cdf)
  ...
  ax.set_xlabel('Attribute')
  ax.set_ylabel('Feature Importance')
  ax.set_title('Random Forest Feature Importance Values to Units Sold')
  plt.xticks( rotation=90)
  plt.axhline(y=0, linewidth=1, color='k', linestyle='dashed')
  ...
  #Correlation Matrix
  plt.figure(figsize=(14, 10))
  dataplot = sns.heatmap(summer_df.corr(), cmap="coolwarm", annot=True, fmt=".2f")

  # displaying heatmap
  plt.title('Correlation Matrix - Summer Product Performance')
  plt.show()

  #create correlation matrix
  success_corr_matrix = summer_df.corr()
  #extract corr values related to total units sold
  success_corr_totalunitssold = pd.DataFrame(success_corr_matrix['units_sold'])
  #reorder from largest to smallest
  success_corr_totalunitssold = success_corr_totalunitssold.sort_values(by='units_sold', ascending=False)

  # Get the number of bars to plot
  num_bars = success_corr_totalunitssold['units_sold'].shape[0]

  # Generate a list of colors from the 'rainbow' colormap
  cmap = plt.get_cmap('cool')
  colors = cmap(np.linspace(0, 1, num_bars))

  ax = success_corr_totalunitssold.plot.bar(y='units_sold', rot=0, legend=False, color=colors, edgecolor = 'k', width = 1)

  # Set labels and title (optional)
  plt.rcParams["figure.figsize"] = (10, 5)
  ax.set_xlabel('Categories')
  ax.set_ylabel('Correlation Value')
  ax.set_title('Attribute Correlation Values to Units Sold')
  plt.xticks(rotation=90)
  plt.axhline(y=0, linewidth=1, color='k', linestyle='dashed')
